...
**Sesión 04/05/2026 (tarde) — Sales Pulse Semanal (PR feat/sales-pulse-weekly):** cierre del ticket de Rod ("Correo automático de Dash IA con el pulso de ventas semanal, todos los lunes") con un Monday Morning Briefing automático tipo Decision Intelligence Platform. Cron lunes 11:30 UTC = 8:30 AM PYT desde `dash@fenixbrands.com.py`. Email con 5 bloques: headline (semana ISO + ventas + WoW% + YoY%), cumplimiento mensual con barra + run-rate vs Budget_2026, top 3 marcas/SKUs/tiendas, alertas (novedades sin distribuir, STH bajo 30-90d, DSO snapshot vs hace 4 semanas), freshness + CTA. Sin LLM en MVP — narrativa por templates puros con thresholds. Arquitectura dual-project: RPC `compute_sales_pulse(date)` SECURITY DEFINER en BD DATA + tablas `sales_pulse_subscribers`/`sales_pulse_runs` + cron `sales-pulse-monday` en BD AUTH. Edge Function `send-sales-pulse` orquesta los dos con auth dual (`x-cron-secret` para cron, JWT super_user para manual). Domain puro espejado en `_shared/salesPulse/` con test sync byte-equivalente. Permiso nuevo `canManageSalesPulse` (super_user). Página admin `/sales-pulse` con 3 secciones: subscribers (CRUD con activate/deactivate), historial expandible (últimos 12 runs con status + recipients + payload jsonb), pruebas (dry-run con preview HTML iframe sandboxed + envío real a email puntual). Seed inicial: Rod (rodrigo@fenixbrands.com.py) + Carlos (carlos@subestatica.com). 1896 tests (+40), typecheck 0, lint 0, build OK. Doc completa en `docs/SESION_2026-05-04_SALES_PULSE.md` + checklist runbook en `docs/SALES_PULSE_DEPLOY.md`. **Pendiente operador:** aplicar `sql/029_sales_pulse.sql` en BD DATA + `sql/030_sales_pulse_auth.sql` en BD AUTH + crear vault secret `sales_pulse_cron_secret` + setear secrets de la EF (`DATA_SUPABASE_URL`, `DATA_SERVICE_ROLE_KEY`, `CRON_SECRET`) + `supabase functions deploy send-sales-pulse` + smoke test desde `/sales-pulse`.
