...
| 10 | SalesPulseAdminPage (`/sales-pulse`) — admin del Sales Pulse Semanal: subscribers + historial + dry-run + envío manual | ✅ COMPLETO sesión 04/05/2026 |

**La app corre:** `npm run dev` → http://localhost:5173
**Tests:** 1896 passing (70 suites) | TSC 0 errores | Build OK | ESLint 0 errores
**Deploy:** https://fenix-brands-one.vercel.app
**Sesión 04/05/2026 (noche) — Triage ticket "On-time delivery" (Carlos):** auditoría BD para confirmar/desmentir el supuesto del usuario ("Derlys carga datos sin avisar"). Probado: `productos_importacion` está vacía hoy (count=0, antes ~580 filas — Edgar/Derlys reorganizando), 34 nombres de columnas candidatas para fecha real (`FECHALLEGADAREAL`, `FECHARECEPCION`, `FECHAINGRESO`, `FECHAARRIBOREAL`, `RECIBIDO`, `DEMORA`, etc.) → 0 existen, audit columns (`updated_at`, `status_changed_at`) → solo `created_at`, ~70 nombres de tablas alternativas en BD DATA + AUTH (`arribos`, `recepciones`, `oc_recepcion`, `fcomprasenc`, `delivery_logs`, `mv_compras`, etc.) → 0 encontradas, proxy via `mv_sth_cohort` bloqueado porque `productos_importacion` no expone `sku_comercial`. **Veredicto:** Carlos tenía razón, dato no existe en BD. Ticket queda en espera de Derlys/Edgar. Spec entregada en `docs/PENDING_DERLYS_OTD_ARRIVAL_DATES.md` con SQL `ALTER TABLE` listo para Derlys (2 columnas TEXT formato D-Mon-YYYY: `FECHALLEGADAREAL` obligatoria + `FECHADESPACHO` opcional), 4 decisiones pendientes para Rodrigo/Carlos (tolerancia ±N días, agrupaciones, umbral semáforo, exclusión ANULADO) y diseño del ticket nuevo (tab "Cumplimiento" en `/logistica`, KPI card OTD%, comparativas proveedor/marca/mes, tabla detalle por OC, ~4-6 hs de implementación una vez destrabado). 0 código tocado en esta sesión, sin commits.

**Sesión 04/05/2026 (tarde) — Sales Pulse Semanal (PR #56 mergeado + 12 commits de iteración en main, todo en producción):** cierre del ticket de Rod ("Correo automático de Dash IA con el pulso de ventas semanal, todos los lunes"). Email firmado por `Dash IA <dash@fenixbrands.com.py>` que dispara cada **lunes 17:00 UTC = 14:00 PYT** (ajustado a pedido del cliente desde el 8:30 AM original). Arquitectura dual-project: RPC `compute_sales_pulse(date)` en BD DATA + tablas `sales_pulse_subscribers`/`sales_pulse_runs` + cron `sales-pulse-monday` en BD AUTH. Edge Function `send-sales-pulse` orquesta los dos con auth dual (`x-cron-secret` cron, JWT super_user manual) y deploy con `--no-verify-jwt`. UI admin completa en `/sales-pulse`: subscribers CRUD + activate/deactivate + remove (super_user), historial **paginado** (12/página, range+count exact, placeholderData prev) con **borrado por fila** y RLS policy nueva, dry-run con iframe sandboxed (no requiere destinatarios), envío manual a email puntual. Email rediseñado byte-a-byte con tokens del DS: paleta brand `#465fff`, gray 50/200/600/900, success-700/error-700/warning-700, StatCard pattern (label uppercase tracking-widest 11px + valor bold tabular-nums), DataFreshnessTag con dot circular, hero con monto gigante 34px + delta pills WoW/YoY. Seeds: Rod (rodrigo@fenixbrands.com.py) + Carlos (carlos@subestatica.com). **Vivos en el email:** headline (sem ISO + ventas + WoW + YoY), cumplimiento mensual (acumulado/meta real desde Budget_2026/proyección run-rate/gap, con barra de progreso tonal), top 3 marcas normalizadas a Martel/Wrangler/Lee (server-side, descarta ruido del ERP tipo "Niella"/"Varias"), freshness con stale flag. **Stub para Fase 2** (timeout sobre fjdhstvta1 280K filas): top 3 SKUs, top 3 tiendas, alertas novedades+STH+DSO. Solución de Fase 2 diseñada en `docs/SALES_PULSE_FASE_2_BACKLOG.md` (MV semanal pre-agregada). Domain puro espejado en `_shared/salesPulse/` con test sync byte-equivalente, 40 tests passing. Permiso nuevo `canManageSalesPulse` (super_user). 12 commits de fixes post-merge en main: cron 14h, imports `.ts`, `--no-verify-jwt`, `pg_catalog.to_jsonb`, `set_config('statement_timeout')`, prefilter year/month, marcas normalizadas, dry_run sin destinatarios, regexp_replace para Budget Revenue, paginación, borrado, rediseño UI. Tests 1896, typecheck 0, lint 0, build OK. Docs: `docs/SESION_2026-05-04_SALES_PULSE.md` (completa con todos los fixes), `docs/SALES_PULSE_DEPLOY.md` (runbook), `docs/SALES_PULSE_FASE_2_BACKLOG.md` (bloques pendientes + diseño). **Estado deploy:** todo aplicado en prod (SQL DATA + AUTH + secrets + EF + Vercel). Cron activo verificable en `cron.job`. Próxima sesión: Fase 2 con `mv_pulso_semanal` cuando Rod baje formato definitivo o pida alertas.

**Sesión 04/05/2026 (mañana):** Carga de markdown por SKU Fase 1 — columna "Promoción" de `/precios` pasa de placeholder a editable. Tabla nueva `sku_markdowns` en BD AUTH (append-only, RLS write=super_user/gerencia, UNIQUE parcial sobre is_active=true). Domain puro `markdown.ts` (applyMarkdown, calcMbpEffective, validators). Modal con preview de PVP efectivo + MBP% recalculado + nota opcional. PVP en tabla muestra efectivo + original tachado cuando hay markdown. Permiso nuevo `canEditPricing`. Status `pending_approval`/`rejected` reservados para Fase 2 (workflow de aprobación al Gte Comercial). 1856 tests (+17 markdown, −3 getPromotionStatus removido). Doc en `docs/SESION_2026-05-04_MARKDOWN.md`. **Pendiente operador:** aplicar `sql/028_sku_markdowns.sql` en Supabase AUTH + `vercel --prod`.
