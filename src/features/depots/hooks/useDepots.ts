...
export function useDepots(): UseDepotsResult {
  ...
  const data = useMemo<DepotData | null>(() => {
    ...
    const allData = buildDepotData(inventoryQ.data, salesHistQ.data);
    ...
    if (filters.brand !== "total") {
      ...
      return buildDepotData(filtered, salesHistQ.data);
    }

    return allData;
  }, [inventoryQ.data, salesHistQ.data, filters.brand]);
