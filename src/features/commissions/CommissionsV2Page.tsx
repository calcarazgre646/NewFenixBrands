/**
 * CommissionsV2Page — nueva sección unificada de Comisiones (PR 2/3).
 *
 * Esta página convive con la vieja `/comisiones` durante PR 2. Se monta en
 * `/comisiones-v2` y queda gateada solo para super_user (acceso QA). PR 3
 * la promueve a `/comisiones` y elimina las rutas viejas.
 *
 * Layout:
 *   Header (título + filtros mes/canal + freshness + tabs)
 *   Contenido por tab (Resumen | Equipo | Histórico)
 */
...
export default function CommissionsV2Page() {
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="inline-flex items-center rounded-full border border-brand-200 bg-brand-50 px-2 py-0.5 text-[9px] font-semibold uppercase tracking-widest text-brand-700 dark:border-brand-500/20 dark:bg-brand-500/10 dark:text-brand-400">
              v2 preview
            </span>
          </div>
          <p className="text-xs text-gray-500 dark:text-gray-400">
            {data.scope === "self"
              ...
              : "Vista unificada del equipo — actual + proyección + histórico"}
          </p>
        </div>
